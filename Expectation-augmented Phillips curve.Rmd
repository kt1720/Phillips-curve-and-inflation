...
# Cpi series
cpiaucsl <- read_csv("CPIAUCSL.csv")
# Core cpi series
cpilfesl <- read_csv("CPILFESL.csv") %>%
  mutate(CPILFESL = as.double(CPILFESL))
# Unemployment rate
unrate <- read_csv("UNRATE.csv")
# Natural rate of unemployment 
nroust <- read_csv("NROUST.csv")
...
```{r}
date <- seq(data$DATE[1], data$DATE[145], by = '+3 month')
...
for(i in t:tt){
  ...
  C = ((RSS - RSS1 - RSS2) / (RSS1 + RSS2)) * ((145 - 2*2) / 2)
  if(C > chow){
    chow = C
    index = i
  }
}
qalpha = qf(0.95,2,145-4)
ifelse(chow>qalpha, 
...
for(i in t:tt){
  ...
  Cc = ((RSS_core - RSS1c - RSS2c) / (RSS1c + RSS2c)) * ((145 - 2*2) / 2)
  if(Cc > chowc){
    chowc = Cc
    indexc = i
  }
...
data_post = subset(data, (label2 == "2008Q3-2019Q4" | label2 == "2020Q1-2021Q1"))

ggplot() +
  geom_point(data = data, mapping = aes(x = un_gap, y = back_cpiaucsl_qa, color = label2, shape = label2)) +
  geom_text(data = data[141:145, ], aes(x = un_gap, y = back_cpiaucsl_qa, label = DATE), position=position_dodge(width=1.5), hjust = 1.5, size = 2.5) +
  geom_text_repel(data = data[141:145, ], aes(x = un_gap, y = back_cpiaucsl_qa, label = DATE), size = 2.5) +
  ...
  coord_cartesian(ylim = c(-14, 5), xlim = c(-2, 8.5)) +
  xlab("Unemployment gap") +
  ylab("CPI inflation surprises") 
```
...
data_postc = subset(data, (label3 == "1991Q1-2019Q4" | label3 == "2020Q1-2021Q1"))

ggplot() +
  geom_point(data = data, mapping = aes(x = un_gap, y = back_cpilfesl_qa, color = label3, shape = label3)) +
  geom_text(data = data[141:145, ], aes(x = un_gap, y = back_cpilfesl_qa, label = DATE), position=position_dodge(width=1.5), hjust = 1.5, size = 2.5) +
  geom_text_repel(data = data[141:145, ], aes(x = un_gap, y = back_cpilfesl_qa, label = DATE), size = 2.5) +
  ...
  coord_cartesian(ylim = c(-4, 3), xlim = c(-1.75, 8.5)) +
